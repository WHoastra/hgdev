...
import { getChannel, getChannelMessages, sendChannelMessage, deleteChannelMessage, updateLastRead, subscribeToChannelMessages, joinChannel, reportChannelMessage } from '../lib/channels'
...
function Channel() {
  ...
  const handleSend = async () => {
    if (!input.trim() || sending || !channel) return
    const text = input; setInput(''); setSending(true)
    const result = await sendChannelMessage(channel.id, userId, text, replyTo?.id)
          ...
          return (
            <div key={msg.id}>
              ...
              <ChannelMessage message={msg} currentUserId={userId} currentUserRole={myRole} showHeader={showHeader}
                onReply={setReplyTo} onDelete={handleDelete} onReport={setReportMsg} />
            </div>
          )
        })}
        <div ref={endRef} />
